...
lecturerSearchBP = Blueprint('lecturerSearch',__name__)

@lecturerBP.route('/main?email=<email>', methods=['GET','POST'])
@login_required
def get_lecturer(email):

    return render_template('lecturer.html',email=email,Wtype="Lecturer",function="Lecturer")

@lecturerSearchBP.route('/Search?email=<email>', methods=['GET','POST'])
def search(email):
    if request.method == 'GET':
        # print(1)
        return render_template('search.html',email=email,function="Search",Wtype="lecturer")
    else:
        ...
        if _type=='courseName':
            ...
            if(show_pre == 1):
                ...
                return render_template('search.html',type=_type,Wtype="lecturer",course=course,email=email,function="SEARCH",show_pre=show_pre,precourseName=precourseName,beprecourseName=beprecourseName)
            return render_template('search.html',type=_type,Wtype="lecturer",course=course,email=email,function="SEARCH")
        elif _type=='CILO':
            ...
            if(len(cilo) == 1):
                ...
                return render_template('search.html',type=_type,Wtype="lecturer",cilo=cilo,email=email,function="SEARCH",show_pre=show_pre,precourse_cilos=precourse_cilos,beprecourse_cilos=beprecourse_cilos,cilo_courseName=cilo_courseName)
            # print(result)
            return render_template('search.html',type=_type,Wtype="lecturer",cilo=cilo,email=email,function="SEARCH",cilo_courseName=cilo_courseName)
        else:
            programme=Course.query.filter(Course.programme == searchMessage).group_by(Course.name).all()
            return render_template('search.html',type=_type,Wtype="lecturer",programme=programme,email=email,function="SEARCH")
